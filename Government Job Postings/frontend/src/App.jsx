...
function App() {
  ...
  const fetchJobs = useCallback(async () => {
    ...
    try {
      ...
      if (data && data.data) {
        setJobs(data.data);
        if (data.data.length === 0 && !syncing) {
          // Trigger sync directly to avoid dependency issues
          setSyncing(true);
          fetch(`${API_BASE}/jobs/sync`, { method: 'POST' }).catch(err => {
            console.error(err);
            setSyncing(false);
          });
        }
  ...
  // Trigger sync is now defined above
    ...
    <div className="dashboard-wrapper">
      ...
      <aside className="sidebar">
        <div className="brand-section">
          <div className="brand-logo" style={{ color: 'white' }}>🎓</div>
          <div className="brand-info">
            <h2>EduOS Jobs</h2>
            <span>Govt Job Portal</span>
          </div>
        </div>
